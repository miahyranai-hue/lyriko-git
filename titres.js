...
const chansons = [
        ...
        titre: "Shape of you", 
        artiste: "Ed Sheeran",
        paroles: `The club isn't the best place to find a lover
        ...
        titre: "Always",
        artiste: "Bon Jovi",
        paroles: `This Romeo is bleeding
        ...
        artiste: "Rosé",
        paroles: `Tell me that I'm special, tell me I look pretty
                ...
                I'd give it all up if you told me that I'd be
                The number one girl in your eyes
                The number one girl in your eye`
                        }
];
...
function afficherArtistes(filtre = "") {
    ...
    chansons.forEach(chanson => {
        if(!titres[chanson.titre]) {
            titres[chanson.titre] = 0;
        }
        titres[chanson.titre]++;
    });

    for (const titre in titres) {
        if (titre.toLowerCase().includes(filtre.toLowerCase())) {
            const card = document.createElement("div");
            card.className = "card-artiste";
            card.innerHTML = `
                <h3>${titre}</h3>
                <p>${titres[titre]} chanteur(s)</p>
            `;
            card.addEventListener("click", () => {
            afficherChansons(titre);
});
...
afficherArtistes();
function afficherChansons(titre) {
    artistesDiv.innerHTML = "";
    ...
    chansons.forEach(chanson => {
        if (chanson.titre === titre) {
            ...
            card.innerHTML = `
                ...
                <i class="fa-heart fa-solid coeur-favori"></i>
            </div>
`;
            

            const coeur = card.querySelector(".coeur-favori");
    ...
    const index = favoris.findIndex(
        f => f.titre === chanson.titre
    );

    if(index === -1){
        favoris.push(chanson);
        coeur.classList.add("actif");
    } else {
        favoris.splice(index, 1);
        coeur.classList.remove("actif");
    }
...
if(favoris.some(f => f.titre === chanson.titre)){
    coeur.classList.add("actif");
}
...
function afficherParoles(chanson) {
    artistesDiv.innerHTML = `
        <div class="card-paroles">
            <h3>${chanson.artiste}</h3>
            <p class="star">${chanson.titre}</p>
            <p class="paroles">${chanson.paroles}</p>
        </div>
    `;
}
