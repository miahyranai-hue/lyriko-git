...
favoris.forEach(chanson => {

    const card = document.createElement("div");
    card.className = "card";
    
    card.dataset.id = chanson.id;
    card.innerHTML = `
        <h3 class="titre">${chanson.titre}</h3>
        <p class="star">${chanson.artiste}</p>

        <i class="fa-solid fa-trash trash-favori"></i>
    `;

    card.querySelector(".trash-favori").addEventListener("click", () => {
...
// // EXEMPLE DE CRÉATION DE CARD (dans favoris)
// chansons.forEach(chanson => {
//   const card = document.createElement("div");
//   card.className = "card";

//   // IMPORTANT : lien entre card et chanson
//   card.dataset.id = chanson.id;

//   card.textContent = `${chanson.artiste} - ${chanson.titre}`;

// //   document.body.appendChild(card); // ou ton container favoris
// });
...
// CLIC SUR UNE CARD (fonctionne même si JS crée les cards)
